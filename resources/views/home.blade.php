        ...
        <div class="col">
            ...
            <form action="{{route('pastes.store')}}" method="POST">
                ...
                <select class="form-select mt-3" id="status" name="status">
                    <option selected value="{{\App\Enums\PastaStatusEnum::PUBLIC->value}}">Выбрать параметры доступа (по
                        умолчанию public, видна всем)
                    </option>
                    <option value="{{\App\Enums\PastaStatusEnum::UNLISTED->value}}">unlisted (доступ по ссылке)</option>
                    <option value="{{\App\Enums\PastaStatusEnum::PRIVATE->value}}">private (доступна только автору)
                    </option>
                </select>

                <input type="hidden" name="short_link"
                       value="{{mb_strimwidth(md5(random_int(1, 1000000)), 1, 8)}}">
